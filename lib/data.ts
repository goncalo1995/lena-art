...
// Helper: Create static supabase client for SSG (no cookies)
function getStaticSupabase() {
  const { createStaticClient } = require("./supabase/static")
  return createStaticClient()
}

// ========== STATIC DATA FETCHING (for SSG - no cookies) ==========

export async function getListingItemsStatic(
  artType: ArtType,
  locale: string
): Promise<ListingItem[]> {
...
export async function getCollectionBySlugStatic(
  slug: string,
  artType: ArtType,
  locale: string
) {
  ...
  if (supabase) {
    ...
    if (collection) {
      ...
      const localizedArtworks = (artworks || []).map((a: any) =>
        ({
          ...a,
          title: localizedField(a as any, locale, "title"),
          short_description: localizedField(a as any, locale, "short_description"),
...
export async function getArtworkInCollectionStatic(
